...
# Modified by NothingnessN / XDA Developers (aLoNeDaRk1121)
...
ARCH=arm64
BUILD_CROSS_COMPILE=~/tct/bin/aarch64-linux-android-
...
PAGE_SIZE=2048
DTB_PADDING=0
KERNELNAME=DumP_KerneL
KERNELCONFIG=DumP_KerneL
ZIPLOC=zip
RAMDISKLOC=ramdisk
...
rm -f $RDIR/build/build.log
MODEL=on7xelte
KERNEL_DEFCONFIG=DumP_KerneL_on7xelte_defconfig
START_TIME=`date +%s`
	(
	FUNC_BUILD_BOOTIMG
	) 2>&1	 | tee -a $RDIR/build/build.log
...
echo ""
echo " ${bldblu} 1) Build DumP-KerneL boot.img for J7 Prime ${txtrst}"
echo ""
